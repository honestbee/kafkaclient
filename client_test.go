...
func TestNewClient(t *testing.T) {
	...
	for _, tc := range testCases {
		client, err := NewClient(tc.brokers, tc.config, tc.monitorer)
...
func TestNewRetryableConsumer(t *testing.T) {
	...
	consumer, err := client.newRetryableConsumer("my_group", []string{"my_topic"}, calc, 2, operate, newTestConsumer, newTestAsyncProducer)
	testingutil.Ok(t, err)

	for i := 0; i < len(consumer.retriers); i++ {
		producer := consumer.retriers[i].producer.(*MockAsyncProducer)
	...
	producer := consumer.producer.(*MockAsyncProducer)
	producer.On("Close").Once().Return(nil)
